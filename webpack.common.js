...
const env = JSON.stringify(process.env.NODE_ENV).replace(/\"/g, '').replace(' ', '');
console.log('===============================');
...
const pathConfig = {
    ...
    production: {
        chipPath: `/project/zt/${year}/${name}`,
        publicPath: `//images.koolearn.com/shark/project/zt/${year}/${name}/`,
...
try {
    ...
    if (fs.existsSync('./src/chip')) {
        chips = fs.readdirSync('./src/chip').filter(name => /\.ejs$/.test(name))
    }
    if (chips.length) {
